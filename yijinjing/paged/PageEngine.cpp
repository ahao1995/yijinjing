...
int PageEngine::reg_journal(const string& clientName)
{
    ...
    {
        KF_LOG_ERROR(logger, "cannot find the client in reg_journal:"<<clientName);
        return -1;
    }
...
{
    KF_LOG_INFO(logger, "[RegClient] (name)" << clientName << " (writer?)" << isWriter<<" (pid)"<<pid);
...
IntPair PageEngine::register_strategy(const string& strategyName)
{
    ...
    KF_LOG_INFO(logger, "[RegStrategy] (name)" << strategyName << " (rid)" << info.rid_start << "-" << info.rid_end<<" (pid)"<<info.pid);
